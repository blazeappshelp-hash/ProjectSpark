...
export interface SectionCardProps {
  ...
  className?: string;
  onBreak?: () => void;
  /** "light" = white bg + black text (default). "dark" = dark bg + white text. */
  variant?: "light" | "dark";
}
interface FallState {
  active: boolean;
  ...
  return (
    <svg
      ...
      style={{
        ...
        animation:
          "debris-fall 0.8s cubic-bezier(0.55, 0, 1, 0.45) 0s forwards",
      }}
    >
...
const CrackLines = ({ level, dark }: { level: number; dark?: boolean }) => {
...
export const BreakableCard = ({
  ...
  return (
    <div className={cn("relative w-full h-full", className)}>
      ...
      <div
        className={cn(
          "absolute inset-0 border-4 border-dashed flex flex-col items-center justify-center z-0",
          dark ? "bg-zinc-100 border-zinc-300" : "bg-zinc-900 border-zinc-700",
        )}
      >
        <span
          className={cn(
            "font-black text-2xl uppercase tracking-widest mb-2",
        ...
        {isBroken && (
          <div
            className={cn(
              "w-24 h-4 border-2 relative overflow-hidden mt-2",
              dark
          ...
          >
            <div
              className={cn(
                "absolute inset-0 transition-all duration-75 ease-linear",
      ...
      <div
        ...
        className={cn(
          "relative z-10 border-4 p-6 shadow-[4px_4px_0px_0px_rgba(0,0,0,0.6)] cursor-grab active:cursor-grabbing select-none h-full flex flex-col justify-between overflow-hidden transition-[background-color]",
          dark
            ? "bg-zinc-900 border-white/80 text-white"
            : "bg-white border-black text-black",
          !isDragging &&
            !isBroken &&
            "hover:animate-[hover-wiggle_0.8s_ease-in-out_infinite]",
          isBroken && "pointer-events-none",
          isFlashing && (dark ? "bg-zinc-800" : "bg-red-50"),
        )}
        style={{
          transform: isBroken
            ? fallTransform
      ...
      >
        <div>
          <div className="flex justify-between items-start mb-4">
            <h3
              className={cn(
                "font-black text-3xl uppercase tracking-tighter leading-none",
        ...
        <div
          className={cn(
            "mt-8 border-t-2 pt-2 flex justify-between text-xs font-mono uppercase",
            dark ? "border-white/40 text-white/70" : "border-black text-black",
          )}
        >
          <span>Fig. {figNum.toString().padStart(2, "0")}</span>
          <span className={cn(crackLevel > 0.7 && "text-red-400 font-bold")}>
            {crackLevel > 0 ? `${Math.round(crackLevel * 100)}% DMG` : "INTACT"}
          </span>
        </div>
        <CrackLines level={crackLevel} dark={dark} />
        {crackLevel > 0.2 && (
          <div
